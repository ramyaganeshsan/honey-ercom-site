...
} = require("../services/checkoutTest.services");
const {
...
const verifyTamaraToken = (token) => {
  ...
  try {
    console.log("entered into the vertify token funcction");
...
exports.registerTamaraWebhook = async (req, res, next) => {
  const TAMARA_API_TOKEN = process.env.TAMARA_API_TOKEN;
  const webhookUrl =
    "https://f94c-2401-4900-8827-5716-38d3-502-cb49-3b82.ngrok-free.app/api/checkoutTest/tamara-webhook";
...
exports.handleTamaraWekhook = async (req, res, next) => {
  ...
  if (!tamaraToken) {
    ...
    console.log("TAMARA TOKEN is missing");

    return res.status(401).send("Authorization token missing");
  }
  const eventBeforeVerifyToken = req.body;
  console.log("eventBeforeVerifyToken : ", eventBeforeVerifyToken);
  // logger.info(eventBeforeVerifyToken);
...
exports.DHLRating = async (req, res, next) => {
  try {
    ...
    console.log("enter into the after api");
...
//api-mock.dhl.com/mydhlapi/rates
